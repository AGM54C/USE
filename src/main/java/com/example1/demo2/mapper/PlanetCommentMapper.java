...
public interface PlanetCommentMapper {
     ...
     * 插入点赞记录
     */
    @Insert("INSERT INTO tab_planet_comment_like(user_id, planet_comment_id, create_time) " +
     ...
     * 删除点赞记录
     */
    @Delete("DELETE FROM tab_planet_comment_like WHERE user_id = #{userId} AND planet_comment_id = #{commentId}")
    void deleteLike(@Param("userId") Integer userId, @Param("commentId") Integer commentId);

    /**
     * 检查是否已点赞
     */
    @Select("SELECT COUNT(*) > 0 FROM tab_planet_comment_like WHERE user_id = #{userId} AND planet_comment_id = #{commentId}")
     ...
     * 删除评论的点赞记录
     */
    @Delete("DELETE FROM tab_planet_comment_like WHERE planet_comment_id = #{commentId}")
    ...
    @Delete("DELETE FROM tab_planet_comment_like WHERE planet_comment_id IN " +
            "(SELECT planet_comment_id FROM tab_planet_comment WHERE parent_comment_id = #{commentId})")
    void deleteChildCommentLikes(Integer commentId);

    /**
     * 删除评论的所有子评论
     */
    @Delete("DELETE FROM tab_planet_comment WHERE parent_comment_id = #{commentId}")
    void deleteChildComments(Integer commentId);

    /**
     * 获取评论的所有子评论ID
     */
    @Select("SELECT planet_comment_id FROM tab_planet_comment WHERE parent_comment_id = #{commentId}")
    List<Integer> getChildCommentIds(Integer commentId);

    /**
     * 删除用户的所有星球评论
     */
    @Delete("DELETE FROM tab_planet_comment WHERE user_id = #{userId}")
    void deleteCommentsByUserId(Integer userId);

    /**
     * 删除用户的所有星球评论点赞记录
     */
    @Delete("DELETE FROM tab_planet_comment_like WHERE user_id = #{userId}")
    void deleteLikesByUserId(Integer userId);

    /**
     * 获取用户的所有星球评论ID
     */
    @Select("SELECT planet_comment_id FROM tab_planet_comment WHERE user_id = #{userId}")
    List<Integer> getCommentIdsByUserId(Integer userId);

    /**
     * 删除与评论相关的通知
     */
    @Delete("DELETE FROM tab_notification WHERE target_type = 4 AND target_id = #{commentId}")
    ...
    void deleteLikesByPlanetId(String planetId);
}